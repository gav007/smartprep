// src/components/tools/ResistorCalculator.tsx
'use client';
...
import { AlertCircle, Palette, Sigma, Thermometer, HelpCircle, RotateCcw, Copy } from 'lucide-react'; // Using Palette icon
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import CalculatorCard from '../calculators/CalculatorCard'; // Import shared card
import CalculatorInput from '../calculators/CalculatorInput'; // Import shared input (for value->band mode)
import { calculateResistorFromBands, valueToResistorBands, ResistorColorMap } from '@/lib/calculator-utils'; // Fixed import path
...
export default function ResistorCalculator() {
    ...
    const renderBandSelect = (
        ...
        const colorMap = bandKey === 'multiplier' ? multiplierColorsMap :
                         bandKey === 'tolerance' ? toleranceColorsMap :
                                ...
                                <SelectContent>
                                    ...
                                    {availableColors.map(color => {
                                        ...
                                        if (bandKey === 'band1' || bandKey === 'band2' || (bandKey === 'band3' && showBand3Digit)) {
                                            ...
                                            (bandKey === 'band3' && band3IsMultiplier) || // 4-band multiplier
                                            (bandKey === 'multiplier' && band4IsMultiplier) // 5/6-band multiplier
                                        ) {
                                            displayValue = formatDisplayMultiplier(multiplierColorsMap[color]);
                                        } else if (
                                            (bandKey === 'multiplier' && band4IsTolerance) || // 4-band tolerance
                                            (bandKey === 'tolerance' && band5IsTolerance) // 5/6-band tolerance
                                        ) {
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Band Selectors Grid */}
                         <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                            ...
                            {band3IsMultiplier && renderBandSelect('band3', 'Multiplier', 'Multiplier value (4-band).', multiplierColors, true)} {/* Label changed */}
                             {/* Band 4: Multiplier or Tolerance */}
                             {band4IsMultiplier && renderBandSelect('multiplier', 'Multiplier', 'Multiplier value (5/6 band).', multiplierColors, true)} {/* Label changed */}
                             {band4IsTolerance && renderBandSelect('multiplier', 'Tolerance', 'Tolerance percentage (4-band).', toleranceColors, true)} {/* Label changed */}
                             {/* Band 5: Tolerance */}
                             {band5IsTolerance && renderBandSelect('tolerance', 'Tolerance', 'Tolerance percentage (5/6 band).', toleranceColors, true)} {/* Label changed */}
